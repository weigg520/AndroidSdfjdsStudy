...
public class LinkWifi {
    ...
    public LinkWifi(WifiManager manager) {
        wifimanager = manager;
    }

    public static LinkWifi getInstance(WifiManager manager) {
        if (mLinkWifi == null) {
            mLinkWifi = new LinkWifi(manager);
        }
        return mLinkWifi;
    }
        ...
        for (int i = 0; i < wifiConfigList.size(); i++) {
            ...
            if (wifi.networkId == wifiId) {
                while (!(wifimanager.enableNetwork(wifiId, true))) {//激活该Id，建立连接
                    ...
                    //LogUtils.e("ConnectWifi", String.valueOf(wifiConfigList.get(wifiId).status));
